// This file is required because maintenance orders require more fields for their units, including file uploads
$(function() {
...
function toggle_submit_button_state(has_units) {
    ...
    $('#submitforallocation_button,#tobescheduled_button').on('click', function(event) {
        // Does the order have any Transport units?
                ...
                $( "#dialog-confirm" ).dialog({
                    ...
                    buttons: {
                        "Use this address": function() {
                            ...
                            $('#order_edit_form').append(clicked_button_input);
...
function add_unit(unit) {
    ...
    $(new_row).attr('data-order_id', unit.order_id);
...
function delete_unit_location_document(button) {
    if (confirm('Are you sure you want to delete this photo?')) {
        ...
        $.post(base_url+'miniant/orders/order_ajax/delete_unit_location_document/'+unit_id, function(data) {
            print_message(data.message, data.type);
        },'json');
...
function submit_edit_unit() {
    ...
    if (errors_found.length == 0) {
        $.post(base_url+'miniant/orders/order_ajax/edit_unit', {
                ...
                order_id: $('#edit_unit_form input[name=order_id][disabled!="disabled"]').val(),

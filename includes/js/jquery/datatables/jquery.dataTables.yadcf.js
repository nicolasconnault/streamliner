...
* html_data_type
				...
				Type:				String
				Default value:		text
				Possible values:	text / value / id / function
	...
	function addCustomFunctionFilterCapability(table_selector_jq_friendly, filterId, col_num) {
		...
		$.fn.dataTableExt.afnFiltering.push(
			function (oSettings, aData, iDataIndex, rowData) {
				...
				retVal = custom_func(filterVal, columnVal, col_num, rowData[col_num]);
...
function addRangeNumberSliderFilterCapability(table_selector_jq_friendly, fromId, toId, col_num, ignore_char) {
	...
	function addCustomFunctionFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label) {
        ...
        // EDIT: Disabling bizarre, buggy "Select date span" markup for custom functions, replacing with basic text input
        ...
        $(filter_selector_string).append("<input type=\"text\" id=\"yadcf-filter-" + table_selector_jq_friendly + "-" + column_number + "\" class=\"yadcf-filter\" onclick='yadcf.stopPropagation(event);"
            + "' placeholder='" + filter_default_label + "'" + " filter_match_mode='contains' id=\"" + filterId + '" onKeyup="yadcf.textKeyUP(\'' + table_selector_jq_friendly + '\', event);"></input>');
						...
						if (column_data_type === "html") {
							...
							for (k = 0; k < col_inner_elements.length; k++) {
								...
								case "id":
									...
									break;
								default:
                                    if (typeof(html_data_type) == 'function') {
                                        col_inner_data = html_data_type(col_inner_elements[k]);
                                    } else {
